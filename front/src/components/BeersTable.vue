  ...
  export default {
    ...
    methods: {
      ...
      refreshBeers() {
        ...
        let url = `api/beers?itemsPerPage=${this.pagination.rowsPerPage}&page=${
          this.pagination.page
          }&order[${this.pagination.sortBy}]=${direction}&name=${
          this.search.name
          }&brewer.country.id=${this.search.country}&price[gte]=${
          this.search.priceFrom
          }&price[lte]=${this.search.priceTo}&type.id=${this.search.type}`
        ...
        this.$http
          ...
          .then((response) => {
            ...
            if (response.data['hydra:view']['hydra:last'] !== undefined) {
              this.lastPage = this.getPageParameters(
